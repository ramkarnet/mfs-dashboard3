...
def ramkar_scan_single(symbol: str, xu100_close, xu100_ema50) -> Tuple[Optional[Dict], bool]:
    try:
        ...
        # ================================
        # RKP - RAMKAR KALİTE PUANI (Tie-Breaker)
        # Sadece 6/6 hisseler arasında sıralama için
        # Konsensüs: ChatGPT + Gemini + Claude
        # ================================
        # 1. ADX Normalizasyon (Max 40 tavan - aşırı gerilmiş trendi ödüllendirme)
        adx_n = min(last_adx, 40) / 40.0
        
        # 2. MFI Normalizasyon (50 altı = 0, 50-80 arası lineer)
        mfi_n = max(0, min((last_mfi - 50) / 30, 1))
        
        # 3. EMA Mesafe Normalizasyon (Risk cezası, max 30)
        ema_n = max(0, min(abs(ema_dist) / 30, 1))
        
        # 4. RKP Kompozit Skor (0-1 arası)
        # %50 Trend Gücü + %30 Para Girişi - %20 Gerilme Cezası
        rkp = (0.5 * adx_n) + (0.3 * mfi_n) - (0.2 * ema_n)
        ...
        return {
            ...
            'score': score,
            'score_pct': score_pct,
            'rkp': round(rkp, 4),  # RAMKAR Kalite Puanı
            'status': status,
...
def run_full_scan(progress_callback=None):
    ...
    # Sıralama: Önce Score, sonra RKP (Kalite Puanı)
    ...
    if regime == "OFF-KILL":
        ...
        if st.session_state.scan_results:
            ...
            for i, r in enumerate(results[:10]):
                ...
                # RKP rengi (kalite göstergesi)
                rkp_color = "#00E676" if r['rkp'] >= 0.5 else "#ffc107" if r['rkp'] >= 0.3 else "#ff1744"
                ...
                <div style="background: #1a1a2e; border-left: 5px solid {r['status_color']}; 
                    ...
                    <div style="display: flex; justify-content: space-between; margin-top: 8px; font-size: 0.85rem; color: #aaa;">
                        ...
                        <span>RKP: <b style="color:{rkp_color}">{r['rkp']:.2f}</b></span>
                        <span>ADX: {r['adx']:.0f}</span>
                        <span>MFI: {r['mfi']:.0f}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-top: 4px; font-size: 0.8rem; color: #666;">
                        ...
                        <span>{'✅' if r['trend_ok'] else '❌'}{'✅' if r['power_ok'] else '❌'}{'✅' if r['vol_ok'] else '❌'}{'✅' if r['sar_ok'] else '❌'}{'✅' if r['dist_ok'] else '❌'}{'✅' if r['bist_ok'] else '❌'}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with st.expander("📋 Tüm Sonuçlar"):
                ...
                df_show.columns = ['Hisse', 'Fiyat', 'Skor', 'RKP', 'Durum', 'ADX', 'MFI', 'Mesafe%', 'ATR%']
